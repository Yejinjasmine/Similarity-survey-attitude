...
# 파일 경로
PAIR_FILE = "Set_2_Attitude_Pairs.csv"
...
# 3단계: 설명 및 동의
    ...
    explanations = [
        "총 **25개의 문장**으로 구성된 **435쌍 (=30 x 29 / 2)**의 문장쌍을 평가합니다.",
        ...
        "위험 추구(risk loving, risk taking)는 자신의 특정한 행동이 처벌이나 손실을 초래할 가능성이 있다는 것을 알고 있음에도 그러한 행동을 하려는 성향으로 정의됩니다. 예를 들어, 조건부 자울 주행 중 돌발 상황에 대비하지 않고 과도하게 편안해하는 것은 “위험 추구” 태도의 입니다.",
        "위험 회피(risk averse)는 자신의 특정한 행동이 처벌이나 손실을 초래할 가능성이 있다는 것을 알고 그러한 행동을 기피하려는 성향으로 정의됩니다. 예를 들어, 조건부 자율 주행 중 제어권 전환 요청이 올 수 있다는 전제를 갖고 긴장 상태를 유지하는 것은 “위험 회피” 태도의 예입니다.",
        "이번 설문에서는 서울대학교에 재학 중인 한국어 모국어 화자들이 참여 예정입니다. 과업을 완료하는 데에는 2시간이 소요될 예정이니, 설문 중 응답을 중지하는 일이 없도록 집중할 수 있는 공간과 시간대에서 응답을 부탁드립니다.",
